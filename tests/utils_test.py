...
def test_align_ao_phase_rejects_large_boundary_jumps():
    """Cycle-boundary phase jumps should be bounded after alignment."""
    fs = 100
    t = np.arange(0, 6, 1 / fs)
    signal = 10 * np.sin(2 * np.pi * 1.0 * t)
    filtered = bandpass_filter(signal, fs=fs)

    # Base phase in [-pi, pi], then inject alternating per-cycle offset.
    base_phase = np.mod(2 * np.pi * 1.0 * t, 2 * np.pi) - np.pi
    ao_phase = base_phase.copy()
    cycle_len = fs
    for i in range(0, len(t), cycle_len):
        if (i // cycle_len) % 2 == 1:
            ao_phase[i : i + cycle_len] = np.angle(
                np.exp(1j * (ao_phase[i : i + cycle_len] + np.pi / 2))
            )

    aligned, _, _ = align_ao_phase(
        filtered,
        ao_phase,
        dt=1 / fs,
        ao_error_threshold=2.0,
        boundary_jump_threshold=np.pi / 2,
    )

    wrapped_step = np.abs(np.angle(np.exp(1j * np.diff(aligned))))
    assert np.max(wrapped_step) <= np.pi / 2 + 1e-6
